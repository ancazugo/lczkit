# Status — what has been built, and what each phase measured

The development record, moved out of the README so the front door can be a front door. It is
kept **verbatim**: every measurement, every reversal and every caveat below was written with
the number in front of it, and this project's own history is of summaries that drifted from
what was measured.

`CLAUDE.md` is the specification and the authority; this is the narrative of what was built
against it, phase by phase, with the experiment write-ups in `docs/experiments/` behind each.
...
See [`docs/experiments/phase-6.6-footprint-attrition.md`](experiments/phase-6.6-footprint-attrition.md).
...
[`docs/experiments/phase-6.7-instrument-diagnostics.md`](experiments/phase-6.7-instrument-diagnostics.md).
...
[`docs/experiments/phase-8-scaling.md`](experiments/phase-8-scaling.md).
...
- **The default cascade is `coarse`: WSF-3D and GHS-BUILT-H on, Open Buildings 2.5D off.**
  ...
  [Phase 10](experiments/phase-10-height-cascade.md). One flag switches it back on.
...
- **The labels do not reproduce each other.** WUDAPT and So2Sat agree at a median 79.7% over the
  twenty-eight cities, ranging 26.3% (Cairo — *below* its own 52.1% majority-class baseline) to
  97.7% (Istanbul). Where two expert label sets disagree, no classifier can agree with both: that is
  a **floor under every residual this package reports**, and it belongs beside any per-city figure.
  By region it is **Europe 91.0% against everything else 71.6%** — East Asia 72.3%, North America
  70.8%, West Asia 69.2%, Africa 61.8%, South Asia 59.5% — so the line is Europe against everywhere
  else, not Global North against Global South. That grouping previously read "Europe + N. America"
  and rested on Vancouver alone; it reorganised as soon as North America grew past one city, and the
  corrected version then held when East Asia went from one city to six.
...
53.2%. [Phase 9](experiments/phase-9-multi-city.md) re-measured both.
...
([Phase 13](experiments/phase-13-bsf-published-ranges.md)): **building surface fraction does not
...
The write-ups in [`docs/experiments/`](experiments/) record how those numbers got there.
[Phase 6.5](experiments/phase-6.5-unit-scale.md) tests the obvious explanation for the original
...
the metric. [Phase 6.6](experiments/phase-6.6-footprint-attrition.md) fixes that.
[Phase 6.7](experiments/phase-6.7-instrument-diagnostics.md) wires the real reference and
...
See [Phase 11](experiments/phase-11-unit-decision.md).

[Phase 8](experiments/phase-8-scaling.md) is what made that measurable, and is the reason the
