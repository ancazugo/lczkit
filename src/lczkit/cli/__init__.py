...
`lczkit.output.export_gis`, `morphometrics raster` calls
`lczkit.morphometrics.raster.refresh_raster`. No command computes anything, and none of them is
the only way to reach what it wraps: the library API is unchanged and the experiment drivers in
`scripts/` still call it directly.
"""
...
from lczkit.cli import morphometrics, site
...
app.add_typer(morphometrics.app, name="morphometrics")
