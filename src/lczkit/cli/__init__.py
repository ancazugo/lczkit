...
`lczkit.viz.build_site`, `site serve` calls `lczkit.viz.serve`. No command computes anything, and
none of them is the only way to reach what it wraps: the library API is unchanged and the
experiment drivers in `scripts/` still call it directly.
"""
